...
class AVDataset(Dataset):
    ...
    def subsample(self, max_pairs, seed=0):
        """Randomly subsample down to at most max_pairs pairs (in place).

        sentence_transformers.fit() builds one in-memory datasets.Dataset from the
        full training set to fingerprint it for caching; pyarrow's default `string`
        column type caps out at ~2GB (32-bit offsets), which very large corpora
        (e.g. PAN2020's ~275k-pair/9GB train split) overflow with an
        ArrowInvalid: offset overflow error. Subsampling keeps well under that.
        """
        if max_pairs is None or self.data_len <= max_pairs:
            return
        idxs = random.Random(seed).sample(range(self.data_len), max_pairs)
        self.data = [self.data[i] for i in idxs]
        self.raw_data = [self.raw_data[i] for i in idxs]
        self.data_len = len(self.data)
...
def run_final_evaluation(model, test_dataset, distance_metric, kernel_fn, args, run_name):
    """Evaluate `model` on `test_dataset`, write results.txt, and (if configured)
    upsert the metrics into the central results CSV.

    Mirrors ContrastiveEvaluator's 'mean' scoring: for each pair, chunk both
    texts, embed the chunks, average the pairwise distances, then turn that
    average distance into a similarity with `kernel_fn`.
    """
    model.eval()
    eval_times, cpu_memories, gpu_memories = [], [], []
    avg_distances, truth = [], []
    gpu_device = 0 if str(args.device).startswith('cuda') else None

    with torch.no_grad():
        for i in tqdm(range(len(test_dataset)), desc='final evaluation'):
            chunked = test_dataset.get_chunked(i)
            start_time = time.time()
            batch1 = [sample.texts[0] for sample in chunked]
            batch2 = [sample.texts[1] for sample in chunked]
            emb1 = model.encode(batch1, show_progress_bar=False)
            emb2 = model.encode(batch2, show_progress_bar=False)
            distances = distance_metric(torch.tensor(emb1), torch.tensor(emb2)).numpy()
            eval_times.append(time.time() - start_time)
            cpu_memories.append(sample_cpu_memory_gb())
            gpu_memories.append(sample_gpu_memory_gb(device=gpu_device))
            avg_distances.append(np.mean(distances))
            truth.append(chunked[0].label)

    avg_pred_time_sec = float(np.mean(eval_times))
    mem_stats = summarize_memory_samples(cpu_memories, gpu_memories)
    similarities = kernel_fn(torch.tensor(avg_distances, requires_grad=False)).numpy()
    res = av_metrics(labels=truth, probas=similarities, threshold=0.5)

    results_dir = os.path.join(args.output_path, run_name)
    ...
    if getattr(args, 'central_results_path', None):
    ...
    model.train()
    logging.info(f'final evaluation: {res}')
    return res
...
def main():
    ...
    parser.add_argument('--diff_margin', type=float, default=0.2)#default=0.09)
    parser.add_argument('--same_margin', type=float, default=0.8)#default=0.91)
    ...
    parser.add_argument('--max_train_pairs', type=int, default=None,
                        help='if set, randomly subsample the training set to at most this many pairs '
                             '(AVDataset only) before training -- works around a pyarrow ~2GB-per-column '
                             'string-offset overflow inside sentence_transformers.fit() for very large '
                             'corpora, e.g. PAN2020 at full scale (~275k pairs, ~9GB of train text)')
    ...
    with wandb.init(project=args.wandb_project, config=vars(args)):
        ...
        if args.which_loss_fn in ['AllTriplet', 'BatchHardTriplet']:
            ...
            train_dataset.subsample(args.max_train_pairs, seed=args.seed)
        ...
        if args.model_path == 'bert-base-cased':
            ...
            # Load pretrained model with the modified config
            hf_model = AutoModel.from_pretrained(args.model_path, config=config)

            # Wrap it in SentenceTransformers' Transformer module
            word_embedding_model = Transformer(args.model_path, max_seq_length=args.max_seq_len)
            word_embedding_model.auto_model = hf_model  # override with our custom dropout model
        ...
        if args.evaluate_first:
            ...
            if args.evaluate_only:
                if not args.AA:
                    run_final_evaluation(model, test_dataset, contrastive_dist_metric, kernel_fn, args, run_name)
        ...
        if not args.AA:
            logging.info('running final evaluation for the results file')
            run_final_evaluation(model, test_dataset, contrastive_dist_metric, kernel_fn, args, run_name)
