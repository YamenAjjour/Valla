            ...
            with Pool(processes=self.num_workers) as pool:
                ...
                for _ in range(self.sample_passes): #Lotta: function for different data input
                    for label, text1, text2 in tqdm(self.train_data, 'Step'):
                        async_results[idx] = pool.apply_async(distance, (text1, text2, self.ppm_order, label))
                        idx += 1
                    '''
                    for auth_id, texts in tqdm(self.train_data.items(), 'Step'):
                        for text in texts:
                            # one same and one
                            same_text = random.choice(self.train_data[auth_id])
                            diff_text = random.choice(self.train_data[random.choice(list(self.train_data.keys()))])
                ...
                with open(cache_path, 'w') as f:
                    json.dump(self.classifier_training_data, f)
...
def train_and_eval_model(train_data, test_data, args, sample_passes=1, ppm_order=5, dset_name='test', cache_dir=None,
    ...
    evaluate_model(test_data, logreg, ppm_order, num_workers, args)
...
def eval_sample(txt0, txt1, ppm_order, logreg, true_lbl=None):
    ...
    elapsed = time.time() - start_time
    cpu_memory_gb = sample_cpu_memory_gb()
    gpu_memory_gb = sample_gpu_memory_gb()
    if true_lbl is not None:
        return proba[0][1], true_lbl, elapsed, cpu_memory_gb, gpu_memory_gb
    return proba[0][1]


def evaluate_model(test_data, logreg, ppm_order, num_workers, args):
    ...
    with Pool(processes=num_workers) as pool:
        ...
        probas, true_lbls, pred_times, cpu_memories, gpu_memories = [], [], [], [], []
        for proba, lbl, pred_time, cpu_memory, gpu_memory in probas_and_true_lbls:
            probas.append(proba)
            true_lbls.append(lbl)
            pred_times.append(pred_time)
            cpu_memories.append(cpu_memory)
            gpu_memories.append(gpu_memory)
    avg_pred_time_sec = sum(pred_times) / len(pred_times)
    mem_stats = summarize_memory_samples(cpu_memories, gpu_memories)
    # now calculate metrics and call it good
    fpr, tpr, thresh = roc_curve(true_lbls, probas)
    threshold = valla.utils.eval_metrics.threshold_search(probas, true_lbls)
    fpr, tpr, _ = roc_curve(true_lbls, probas)
    roc_auc = auc(fpr, tpr)
    print("Test AUC:", roc_auc)
    res = valla.utils.eval_metrics.av_metrics(true_lbls, probas=probas)
    print("AV results:", res)
    results = valla.utils.eval_metrics.evaluate_all(true_lbls, probas)
    print("PAN results:", results)
    ...
    # save metrics
    results_txt = os.path.join(args.cache_path, "results.txt")
    with open(results_txt, "w", encoding="utf-8") as f:
        f.write(f"===AV results===\n")
