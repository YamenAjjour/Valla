...
def evaluate_model(model,
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(tqdm(val_loader, desc='evaluating')):
            ...
            # time and measure memory for the inference call only (not the loss computation below)
            start_time = time.time()
            ...
            pred_times.append(elapsed / labels.shape[0])
            cpu_memories.append(sample_cpu_memory_gb())
            gpu_memories.append(sample_gpu_memory_gb(device if use_cuda else None))
        ...
        avg_pred_time_sec = float(np.mean(pred_times))
        mem_stats = summarize_memory_samples(cpu_memories, gpu_memories)
        print(f"Avg prediction time: {avg_pred_time_sec:.6g} s, "
              f"Avg/Max CPU memory: {mem_stats['avg_cpu_memory_gb']:.4f}/{mem_stats['max_cpu_memory_gb']:.4f} GB, "
              f"Avg/Max GPU memory: {mem_stats['avg_gpu_memory_gb']:.4f}/{mem_stats['max_gpu_memory_gb']:.4f} GB")
        results.update({'avg_prediction_time_sec': avg_pred_time_sec, **mem_stats})
...
def evaluate_model_AA(model,
    ...
    with torch.no_grad():
        ...
        for data in tqdm(val_loader, desc='evaluating AA'):
            ...
            # time and measure memory for the full inference call (embedding + comparison to every author profile)
            start_time = time.time()
            # only need one embedding so only pass those toks
            ...
            elapsed = time.time() - start_time
            # test_batch_size is asserted to be 1 for AA evaluation, so this is already a per-sample time
...
def main(args):
    ...
    if use_wandb:
        # wandb.run.name is only auto-generated once the run talks to the backend, so it
        # stays None in offline mode (WANDB_MODE=offline) - fall back to the run id instead.
        run_name = wandb.run.name if wandb.run.name is not None else wandb.run.id
        args.save_model_dir = os.path.join(args.save_model_dir, run_name)
    else:
    ...
    if AA:
        final_results = evaluate_model_AA(model=model,
                                          auth_prof_loader=auth_profile_dataloader,
                                          val_loader=test_dataloader,
                                          use_wandb=use_wandb,
                                          use_cuda=use_cuda,
                                          device=device,
                                          step=step)
        results_section = '===AA results==='
    else:
        final_results = evaluate_model(model=model,
                                       val_loader=test_dataloader,
                                       loss_fn=loss_fn,
                                       use_cuda=use_cuda,
                                       device=device,
                                       use_wandb=use_wandb,
                                       step=step)
        results_section = '===AV results==='

    efficiency_keys = ('avg_prediction_time_sec', 'avg_cpu_memory_gb', 'max_cpu_memory_gb',
                      'avg_gpu_memory_gb', 'max_gpu_memory_gb')

    results_txt = os.path.join(save_model_dir, 'results.txt')
    os.makedirs(save_model_dir, exist_ok=True)
    with open(results_txt, 'w', encoding='utf-8') as f:
        f.write(f"{results_section}\n")
        for k, v in final_results.items():
            if k in efficiency_keys:
                continue
            f.write(f"{k}: {v:.4f}\n")
        f.write(f"\n===Efficiency===\n")
        f.write(f"avg_prediction_time_sec: {final_results['avg_prediction_time_sec']:.6g}\n")
        for k in efficiency_keys[1:]:
            f.write(f"{k}: {final_results[k]:.4f}\n")

    central_results_path = getattr(args, 'central_results_path', None)
    if central_results_path and not AA:
        update_central_results(
            central_results_path,
            method='adhominem',
            dataset=getattr(args, 'dataset', 'gerav'),
            f1_score=final_results['f1'],
            average_time=final_results['avg_prediction_time_sec'],
            average_memory=final_results['avg_cpu_memory_gb'],
        )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_model_checkpoints', default=True) #action='store_true')
